...
if [ "$cache_needs_update" = true ]; then
    ...
    if mkdir "$LOCK_FILE" 2>/dev/null; then
        ...
        if command -v bunx >/dev/null 2>&1; then
            ...
            if [ -n "$pai_usage_output" ]; then
                # Extract input/output tokens, removing commas and ellipsis
                daily_input=$(echo "$pai_usage_output" | awk -F'│' '{print $4}' | sed 's/[^0-9]//g' | head -c 10)
                daily_output=$(echo "$pai_usage_output" | awk -F'│' '{print $5}' | sed 's/[^0-9]//g' | head -c 10)
                # Extract cost, keep the dollar sign
                daily_cost=$(echo "$pai_usage_output" | awk -F'│' '{print $9}' | sed 's/^ *//;s/ *$//')
